	...
	cases := []struct {
		...
		{row: "B21", name: "a background subagent finishing after the turn leaves the pane idle",
			events: []hooks.Event{ev("UserPromptSubmit", ""), ev("PreToolUse", "Task"),
				...
				{Event: "SubagentStop", Background: hooks.BackgroundEnd, BackgroundID: "agent:a1"}},
			want: session.StatusIdle,
			bug:  "the subagent's own tool calls after the Stop mark the pane working, and its SubagentStop only uncounts it, so the pane stays working with nothing running"},
		...
		{row: "C1", name: "the last tool's PostToolUse delivered after the Stop",
			...
			want:   session.StatusIdle,
			bug:    "handleHook applies events in arrival order, so a PostToolUse that lost the race with its Stop marks the idle pane working, with nothing to mark it idle again"},
		{row: "C14", name: "a lost Stop, then the idle nudge",
			...
			want: session.StatusIdle,
			bug:  "idle_prompt is ignored even by a pane still showing working, so a pane whose Stop never arrived shows working forever"},
		{row: "C18", name: "/clear forgets a denial",
			...
			want: session.StatusIdle,
			bug:  "handleHook returns before ResolveBlocked for a SessionStart, so the denial mark ResolveBlocked says SessionStart clears survives it"},
	}
	for _, c := range cases {
