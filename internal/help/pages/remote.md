...
`flockdeck -detach`. On macOS and Linux, one started from a terminal and
detached from the palette still quits when that terminal closes; one started
with `flockdeck -detach` does not. A remote window closing never stops anything.
