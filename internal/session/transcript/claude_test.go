...
func TestAConversationBehindAWorktreesOwnWorkIsFound(t *testing.T) {
	...
	got, err := claudeConversations(claudeHome(), project)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConversationsWithATrailingSeparator(t *testing.T) {
	...
	got, err := claudeConversations(claudeHome(), cwd+string(filepath.Separator))
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 2 {
...
func TestConversationsListsAndSummarises(t *testing.T) {
	...
	got, err := claudeConversations(claudeHome(), cwd)
	if err != nil {
...
func TestConversationsWithNoHistory(t *testing.T) {
	...
	got, err := claudeConversations(claudeHome(), t.TempDir())
	if err != nil {
		t.Fatalf("conversations: %v", err)
	}
	if len(got) != 0 {
...
func TestConversationsDoNotLeakBetweenCollidingSlugs(t *testing.T) {
	...
	got, err := claudeConversations(claudeHome(), dashed)
	if err != nil {
		t.Fatalf("conversations: %v", err)
	}
	if len(got) != 1 {
	...
	got, err = claudeConversations(claudeHome(), scored)
	if err != nil {
		t.Fatalf("conversations: %v", err)
	}
	if len(got) != 0 {
...
func TestConversationsRereadAReplacedTranscript(t *testing.T) {
	...
	if _, err := claudeConversations(claudeHome(), cwd); err != nil {
		t.Fatalf("conversations: %v", err)
	}
...
func TestConversationsSplitAFolderTwoProjectsShare(t *testing.T) {
	...
	for _, c := range []struct{ cwd, want string }{
		...
		got, err := claudeConversations(claudeHome(), c.cwd)
		if err != nil {
...
func TestConversationsOnACaseSensitiveFilesystemStayApart(t *testing.T) {
	...
	got, err := claudeConversations(claudeHome(), asked)
	if err != nil {
		t.Fatalf("conversations: %v", err)
	}
	if len(got) != 0 {
...
func TestConversationsOfferAConversationThatMovedIntoAWorktree(t *testing.T) {
	...
	got, err = claudeConversations(claudeHome(), worktree)
	if err != nil {
		t.Fatalf("conversations: %v", err)
	}
	if len(got) != 3 {
...
func TestARenamedConversationIsListedByItsName(t *testing.T) {
	...
	if got, _ := claudeConversations(claudeHome(), cwd); len(got) != 1 || got[0].Summary != "shipped" {
